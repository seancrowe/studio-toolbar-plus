...
const buildTreeData = (documentLayouts: Layout[], selectedLayoutIds: string[]) => {
...
export const LayoutMultiSelect: React.FC<LayoutMultiSelectProps> = (
  ...
  const { documentState, layoutConfigs, updateLayoutConfig } = useAppStore();

  const handleMultiSelectChange = (updateLayoutIds: string[]) => {

    console.log(updateLayoutIds);

    updateLayoutConfig({ id: layoutConfig.id, layoutIds: updateLayoutIds })
  }
