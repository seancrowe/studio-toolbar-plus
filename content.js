...
async function checkForUpdates() {
  try {
    ...
    const lastNotifiedVersion = localStorage.getItem('toolbarplus_last_notified_version');

    // Fetch the package.json from GitHub's main branch
    const response = await fetch('https://raw.githubusercontent.com/spicy-labs/studio-toolbar-plus/refs/heads/main/manifest.json');
    const packageJson = await response.json();
    console.log(packageJson);
    const githubVersion = packageJson.version;

    // Compare versions and check if we've already notified
    if (githubVersion !== currentVersion && githubVersion !== lastNotifiedVersion) {
